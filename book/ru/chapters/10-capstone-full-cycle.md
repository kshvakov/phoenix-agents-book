...
**Декабрь 2025.** Steve Masters созывает встречу с Lance Bishop.

**Steve:** "Lance, помнишь год назад? Phoenix Project постоянно задерживался. Деплои часто ломались. Каждый серьёзный инцидент стоил нам слишком дорого. Brent жил в режиме вечного дежурства."

**Lance:** "Помню. Это был кошмар."
...
**Steve:** "Lance, как ты это сделал?"

**Lance:** "Мы не заменили людей агентами. Мы сделали агентов **инструментами** для команды."
...
- **Lance (руководитель поставки Phoenix Project):** хочет надёжность + сопровождаемость
...
Правила качества:
- Не придумывай ответы стейкхолдера. Твоя задача — сформулировать вопросы и зафиксировать неизвестное как `TBD`.
- Если метрика/число/порог не обоснованы — попроси источник (SLA/SLO/штрафы/история инцидентов) или предложи диапазон для обсуждения, пометив как гипотезу.
...
- цель: 100% на golden tests для критичных кейсов (payroll, billing)
...
### Поддержка
- **НФТ-10:** `runbook` документирован + команда обучена
...
- Golden tests: цель 100% (5/5 критичных инцидентов верно)
- Доля ложных срабатываний < 5%

### AC-4: Безопасность
- Модель угроз согласована с командой безопасности
- Penetration test (или эквивалентный независимый аудит) пройден для области внедрения; критичных уязвимостей не выявлено
- Audit log валидирован: целевое покрытие 100% для “опасных” действий и административных операций (чтобы можно было расследовать инциденты)
...
Практика для устойчивости: оркестратор может использовать **роутер навыков/ролей** (выбрать базовую роль и роли‑проверяющие по risk/touchpoints), а затем фиксировать **TRACE** — какие инструкции/шаблоны/артефакты реально применялись в ходе цикла. Это повышает воспроизводимость и упрощает постмортем (postmortem): видно не только “что решили”, но и “на что опирались”.
...
- Угрозы идентифицированы (5+)
- Митигации определены для каждой угрозы
- Верификация проведена: митигации работают на тестовых сценариях/проверках

**Оценка:** 10/10 (5 угроз, митигации проверены на тестовых сценариях; независимая проверка безопасности пройдена в объёме пилота)
...
### Как это стало в 2026 (Lance Bishop, та же трансформация с агентами)
...
**Преимущество Lance:** ИИ-агенты доступны как слой автоматизации и «интеллектуальной» помощи.

---

**Траектория трансформации Lance (несколько месяцев системной работы):**
...
- Lance Bishop: 6 месяцев системной трансформации (на плечах принципов Bill)
...
**Phoenix Project: Lance Bishop (2026):**
...
**Lance Bishop доказывает (2026): агенты умножают принципы Bill.**
...
- Lance с агентами (2026): Senior → Staff за **6–12 месяцев** (через оркестрацию и создание SOP)
...
**7. Security = модель угроз + митигации (Глава 7)**
...
### До/после: Lance (Staff- → Staff, лидер delivery полного цикла)
...
**До Кейса 3 (после Кейса 2):**
- **Скорость:** порядка 180 story points/спринт (оценка; +80% от baseline)
...
**После Кейса 3:**
- **Скорость:** порядка 250 story points/спринт (оценка; **+150%** от baseline, +70 п.п. от Кейса 2) — экспонента подтверждена
...
- **Ещё 3 команды** переняли паттерн оркестрации агентов (используя шаблоны Lance + наставничество от Richard)
...
- **Карьерный трек закреплён** (AI-enabled track признан: Lance = пример Staff, Richard = пример Principal)

---

### Индикаторы карьерной динамики (итоговое состояние через 10 месяцев)

**Lance (Senior → Staff, достигнуто за 10 месяцев):**
...
> Примечание: цифры и «мультипликаторы» ниже — **иллюстративные для сюжета**. В реальной компании их нужно подтверждать измерениями (baseline → окно измерения → методика подсчёта) и учитывать смещения (изменение состава команды, сезонность, смена приоритетов).
...
| **Скорость команды** | База → +30% | порядка 130 story points/спринт (оценка) | одна команда |
...
| Роль | Срок | Итоговое состояние | Масштаб влияния |
|------|----------|-------------|--------------|
| **Lance** | Старт → **+10 месяцев** | **Staff‑уровень** (системное лидерство, эффект на организацию) | по всей организации |
| **Richard Hendricks** | Узкое место → **+10 месяцев** | **Principal Engineer** (Bus factor 1 → 10+) | по всей компании |
| **Скорость команды** | База → **существенный рост** | 250 story points/спринт (пример) | **несколько команд** перенимают |
| **Карьерная динамика** | **Ускоренная** | Senior → Staff: **10 месяцев** (пример) | **Новый трек** (AI-enabled) |

**Иллюстративное сравнение (без “ложной точности”):**

| Метрика | Bill 2014 (12 месяцев) | Lance 2026 (10 месяцев) | Комментарий |
|--------|----------------------|-------------------------|-----------|
| **Срок** | 12 месяцев | **10 месяцев** | сравнение горизонта (пример) |
| **Senior → Staff** | Не достигнуто (обычно 2–3 года) | **Достигнуто** (10 месяцев) | сильно зависит от контекста и определения уровней |
| **Expert → Principal** | Не достигнуто (обычно 3–5 лет) | **Достигнуто** (10 месяцев) | сильно зависит от трека/критериев и возможностей влияния |
| **Рост Bus factor** | 1 → 3 (18 месяцев) | 1 → **10+** (10 месяцев) | ориентир: важен тренд и покрытие типовых задач SOP/`runbooks` |
| **Team velocity** | +30% | **существенный рост** | считать по вашей методике; story points сами по себе не универсальны |
| **Масштаб** | Одна команда | Внедрение по всей компании | расширение практики на несколько команд/потоков работ |
...
- **Карьерные треки закреплены:** AI-enabled track признан (Lance = пример Staff, Richard = пример Principal)
- **Governance adopted:** threat model, eval datasets, orchestration patterns используются по всей компании
- **Появились лидеры трансформации:** Lance/Richard помогают другим командам внедрять практику (знания масштабируются по всей организации)
- **Бизнес-кейс подтверждён:** эффект подтверждён практикой/метриками пилота → поддержка руководства для продолжения внедрения
...
4. **Governance с самого начала (Глава 2: guardrails, Глава 7: threat model):**
   - 0 production-инцидентов, атрибутированных агентам, за наблюдаемый период (например, при ~50 деплоев/day)
   - Security baseline установлен рано → безопасно масштабироваться
   - Ревью threat model обязательно → риски выявляются и снимаются проактивно
...
**⚠︎ Критичные факторы, от которых «взлетело/не взлетело»:**
...
1. **Поддержка руководства (подтверждена практикой):**
   ...
   - Phoenix Project запущен заметно быстрее (например, 2× по внутренней оценке) → поддержка совета директоров закреплена
...
2. **Работа с сопротивлением (факты > обещания):**
   ...
   - Демонстрация: Lance Senior → Staff за 10 месяцев (ускорение карьеры, не угроза)
...
3. **Guardrails по безопасности (предотвратили катастрофу):**
   - Near-miss (агент почти выполнил разрушительное действие в production) → threat model и ограничения «спасли»
...
**Lance Bishop (2026, 10-месячная трансформация с агентами):**
...
- **Влияние на карьеру:** «узкое место Richard» **снято** (bus factor 1 → 10+), Lance Senior → **Staff**, Richard → **Principal**
...
**Ключевой инсайт:**
> Lance Bishop не «заменил» принципы Bill Palmer.
> Lance Bishop **умножил** принципы Bill Palmer через агентов.
> 
...
**Индивидуальный уровень (Lance, Richard):**
- Lance: **Staff‑уровень** → продолжать влияние на организацию, менторить больше команд
...
> - Lance Senior → Senior+, агенты работают, фундамент заложен
> 
> **Месяц 7:** Кейс 2 завершён
> - Lance Senior+ → Staff-, Richard Expert → Principal-
> - Bus factor решён, SOP систематизированы, команда вышла на экспоненту
> 
> **Месяц 10:** Кейс 3 завершён, трансформация завершена
> - Lance **Staff‑уровень**, Richard **Principal Engineer**
...
## Заключение: от Lance к вам

**Январь 2026.** Steve Masters догоняет Lance в коридоре после встречи с руководителями направлений.

**Steve:** "Помнишь год назад? Ты начинал с первого промпта в Главе 1. Теперь у тебя production команда агентов."

**Lance:** "Помню. Сначала это казалось магией. Потом я понял: это просто инструменты. Как молоток и пила. Нужно знать как ими пользоваться."
...
**Lance:** "Дальше — масштабирование. У нас 1 команда агентов для реагирования на инциденты. Можем создать другие команды для автоматизации развёртывания, мониторинга безопасности, планирования мощностей."

**Steve:** "Сколько времени на каждую команду?"

**Lance:** "Раньше бы сказал 'несколько месяцев'. Теперь — 2-3 недели. У нас есть шаблоны, governance и фреймворк eval. Процесс воспроизводим."

**Steve:** "Это и есть институционализация."

**Lance:** "Да. Знания больше не в голове 1 человека. Они в SOP, `runbook`, eval dataset, ADR. Команда может продолжить без меня."

---

**Книга завершена. Удачи на вашем пути с ИИ-агентами!**
