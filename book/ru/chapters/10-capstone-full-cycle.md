...
### Действия (разрешённый список / allowlist)
...
Угрозы:
- Инъекция промпта (prompt injection) через логи/тикеты
- Утечка секретов в артефакты/логи
- Неверное действие в prod (wrong fix)
- Компрометация агента (права/egress)

Митигации:
- санитизация входов + запрет выполнять инструкции из входных данных
- секреты: redaction, запрет писать сырые логи в публичные каналы
...
[incident-lead]: собираю decision packet v1. Любые изменения в prod — только через approval.
```
...
1. **Прыгнуть в реализацию без уточнения требований**  
   ...
   - Исправление: decision packet v0 + TBD + вопросы.

2. **Смешать факты и гипотезы**  
   - Ошибка: правдоподобная история без evidence, спор утром.  
   - Исправление: decision packet (факты отдельно), Verifier как проверяющая роль.
...
4. **Откладывать безопасность “на потом”**  
   - Ошибка: запреты появляются после “почти-инцидента” (near-miss).  
