...
См. минимальный контракт пакета решения (место правды): [Приложение C — пакет решения (decision packet): минимальный контракт]({{< relref "appendix/C-process-and-artifacts.md" >}}#decision-packet-contract).
...
- ФТ-5: сформировать пакет решения (факты/гипотезы/проверки/риск/что требует `approval`).
...
- RunbookExecutor: исполняет только операции из разрешённого списка, требует approval при необходимости.
...
- Все “опасные” действия находятся за разрешённым списком+approval.
- Есть журнал аудита.
...
[incident-lead]: собираю пакет решения v1. Любые изменения в prod — только через approval.
```
