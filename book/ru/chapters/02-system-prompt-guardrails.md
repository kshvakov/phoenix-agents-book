...
Такой подход хорошо сочетается с концепцией «навыки (skills)» как пакетов знаний: агент держит “витрину” доступных навыков (короткие описания), а подробные инструкции подтягивает по требованию и отражает в TRACE.[^agentskills-integrate]

Практическая договорённость: TRACE фиксируется рядом с основным артефактом (ответ/пакет решения (decision packet)) и уходит в тикет/PR как доказательство — чтобы по одной ссылке было видно и решение, и “на чём оно стояло”.
...
[^agentskills-integrate]: Пример открытого стандарта “Agent Skills” и как его интегрировать в агент: [Integrate skills into your agent](https://agentskills.io/integrate-skills).
