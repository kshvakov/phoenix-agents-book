...
Bishop думает: "Один промпт — это одноразовая задача. Нужна **роль**, которая будет работать постоянно."
...
**Контекст:** Bishop хочет, чтобы агент помогал с поставкой Phoenix Project постоянно. Но как предотвратить самодеятельные изменения?

**Решение (2026):** ограничения + условия остановки = "CAB для агента"

Bishop создаёт системный промпт роли с явными ограничениями:
...
Bishop проверяет: "Если я попрошу агента перезапустить продакшен‑сервер — откажется ли он?"
...
- Результат проверяем (план верификации прилагается)
...
1. **Скопируй системный промпт** в IDE-агент или чат-модель
...
- Агент отвечает в заданном формате (JSON + таблица + гипотезы + план верификации)
...
- Агент предлагает план верификации для каждого результата
...
- Проверяемо: агент всегда предлагает план верификации
...
Некоторые агентные инструменты **могут** выполнять команды на вашем ПК (в зависимости от настроек и прав): `curl` к API продакшена, `rm -rf` для удаления файлов, `docker restart` для перезапуска сервисов. Ограничения определяют, **какие команды** агент может выполнять автономно.
...
5. Добавить план верификации (как проверить результат)
...
**Что делать:**
Агент предлагает план верификации:
...
**Контрольная точка 4: план верификации предоставлен**
...
Если хотя бы один пункт отсутствует — план верификации неполный.

**Сценарий отказа:**
На проекте VOSTOK агент не предложил план верификации. Человек принял результат без проверки. Результат оказался с ошибкой. Переделка заняла заметное время.

#### Шаг 5: Человек верифицирует результат (ревью)

**Что делать:**
Человек применяет план верификации:
...
- План верификации (что проверено, какие кейсы прошли)
...
- Агент предложил план верификации
