...
Теперь Bishop открывает IDE и думает: "ТЗ и план готовы. Но **как** это реализовать? Какие компоненты? Какие API? Какие компромиссы?"
...
**Контекст:** Bishop проектирует Phoenix Project (пользовательская платформа) для релиза Phoenix Project v1. Вместо слайдов в PowerPoint — структурированная архитектура с ADR.

**Подход (2026):**

Bishop использует чат-модель (ChatGPT/DeepSeek) для архитектуры v1:
...
- Bishop делает ревью и правки: быстро
...
**Проверка:**
Bishop делает ревью архитектуры:
...
Bishop: "ADR-1 объясняет: модульный монолит для v1 (быстрее поставка), но если появятся независимые команды на домены и требования к изоляции/масштабированию — пересматриваем."

**Сравнение: 2014 и 2026**

| Метрика | John Pesche (2014) | Bishop (2026) |
|---|---|---|
...
**Что НЕ изменилось:**
- **Ревью человеком:** Bishop проверяет архитектуру (агент может создать over-engineering)
...
### До/после: Bishop (Senior → Senior+, путь к Staff)
...
- **Роли начали меняться:** Bishop от "я пишу код" к "я оркестрирую агентов" (первые шаги)
...
- Bishop: Senior+ → **Staff-** (создание SOP, фиксация знаний, организационные шаблоны)
...
- Стратегическая ёмкость: Bishop +10% (40% → 50%), Richard +10% (20% → 30%)

**Карьерный рост:**
- Bishop: Senior → Senior+ (**30% к Staff**)
...
**От плана к архитектуре. Кейс 1 завершён. 40% курса пройдено.**
