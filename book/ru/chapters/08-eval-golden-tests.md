...
- **Test runner (тест‑раннер)** — исполнитель, который **проактивно** прогоняет нужные тесты/проверки (в т.ч. golden tests) при изменениях и доводит до зелёного состояния, не ломая смысл тестов.

Важно: это не обязательно “отдельные модели” — это может быть отдельная роль в вашей multi-agent системе или просто формализованный шаг в CI/CD. Ценность — в **разделении ответственности**: Implementer пишет/меняет, Verifier доказывает, Test runner прогоняет и фиксирует результат.
