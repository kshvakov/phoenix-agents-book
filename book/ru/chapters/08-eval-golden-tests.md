...
```json
{
  ...
  "improvement_opportunities": [
    ...
    },
    {
      ...
      "issue": "TTRC 180s (дольше целевых 120s)",
...
- **Средний TTRC:** 111 секунд — цель < 120s
...
- Цель: точность (accuracy) 100% (5/5), средний TTRC < 100s
...
Запусти eval на текущем агенте → измерь метрики:
...
```
Проверяем INC-2024-01-15... OK (верно, 1.8s)
Проверяем INC-2024-02-10... OK (верно, 2.1s)
Проверяем INC-2024-03-15... OK (верно, 1.5s)
Проверяем INC-2024-04-20... FAIL (неверно, 2.3s)
...
Проверяем INC-2025-12-30... OK (верно, 2.0s)

Результаты:
Точность: 90% (18/20 верно)
Доля ложных срабатываний (false positive rate): 10% (2/20 неверно)
Средний TTRC: 1.9 секунды
```
...
Средний TTRC: 1.8s — -0.1s улучшение
...
**С eval-набором (2026):**
- Измерение качества: точность (accuracy) 90%, TTRC 1.9s (объективные метрики)
