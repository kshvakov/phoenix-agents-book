...
Агент проходит eval (точность 90%) → деплой в продакшен → **падает на edge case** (например, network partition или редкий DB deadlock).
...
- Рутинные сценарии (happy path): точность ≥ 95%
- Краевые случаи (edge cases): точность ≥ 70%
...
- Eval script (посчитать метрики: точность, доля ложных срабатываний, TTRC)
...
**В 2026:** eval-набор делает качество **измеримым**. Точность 80% → improvements → 90% → 95%. Каждое улучшение измеримо.
...
- С eval-набором (2026): точность 90%, TTRC <TTRC_SECONDS>; tracking улучшений есть; golden tests блокируют деплой при регрессии
