...
**Ночь, пятница.** У Bill Palmer звонит телефон.

«Payroll‑система недоступна. Мы не можем провести payroll. Это бизнес‑критично».

В 2014 Bill Palmer столкнулся с этим инцидентом.
...
## Та же задача в 2026: как инженер решает с eval-набором

**Контекст:** инженер в Parts Unlimited 2026 развернул агента для реагирования на инциденты (главы 5-7). Агент работает, тушит рутинные инциденты.

**Вопрос:** Насколько качественно агент работает? Можно ли **измерить**?

В 2014 Bill "чувствовал" что стало лучше. В 2026 инженер **измеряет** через eval-набор.
...
В 2014 Bill решал инциденты, но не учился систематически (знания в голове). В 2026 eval-набор делает обучение **измеримым**: качество можно проверять, видеть регрессии и улучшать плейбуки.
...
**Нотация плейсхолдеров:** в примерах ниже используются значения‑заглушки в угловых скобках (например, `<TIME>`, `<VALUE>`, `<WINDOW>`, `<THRESHOLD>`). Замените их на реальные значения/пороги из вашей системы.
...
**Метрики:**
- Подключения к БД (DB connections): <VALUE>
- Реплики сервиса (service replicas): <VALUE>
- Запросы (queries): <VALUE>
...
**Эталонная первопричина:** ротация логов (log rotation) не работает (логи накопились)
...
**Метрики:**
- API latency: <VALUE> (по сравнению с baseline <VALUE>)
- DB query duration: <VALUE> (по сравнению с baseline <VALUE>)
...
- **Точность (accuracy):** 80% (4/5 correct) — цель ≥ 80%
- **Доля ложных срабатываний (false positive rate):** 20% (1/5 wrong) — цель ≤ 20%
...
- Цель: точность (accuracy) 100% (5/5), средний TTRC < <TTRC_TARGET>

**Время выполнения:**

- В 2014 Bill: нет eval (не измерял качество, «чувствовал»)
- В 2026 eval-набор: 30 минут на подготовку + 10 минут на прогон → измеримое качество + возможности для улучшений
...
**Precision (precision) (для задач с «позитивным классом»):**
```
...
1. **Baseline:** запустить eval на текущем агенте → точность (accuracy) 80%
2. **Улучшение:** добавить новую гипотезу в плейбук триажа
3. **Re-eval:** снова запустить eval → точность (accuracy) 90%
4. **Измерение:** улучшение = +10% (измеримо)

В 2014 Bill "чувствовал" улучшение. В 2026 eval-набор **измеряет** улучшение.

### Концепция 2: Golden tests — защита от регрессий

**Проблема:**

Агент v1: точность (accuracy) 90% (18/20 инцидентов — верно).

Разработчик меняет плейбук триажа (оптимизация).

Агент v2: точность (accuracy) 75% (15/20 инцидентов — верно).
...
В 2014 Bill не мог предотвращать регрессии (нет eval). В 2026 golden tests блокируют деплой, если регрессия обнаружена.

#### Verification agent и test runner: роли, которые делают качество “реальным”
...
- **Verification agent (верификатор)** — независимый “скептик”, который проверяет, что заявленное “готово” действительно работает: запускает релевантные проверки, ищет пропуски и edge cases, возвращает отчёт “что прошло / что сломано / что не проверено”.
- **Test runner (тест‑раннер)** — исполнитель, который **проактивно** прогоняет нужные тесты/проверки (в т.ч. golden tests) при изменениях и доводит до зелёного состояния, **не переписывая ожидания тестов под “как получилось”** и не ломая их исходный смысл.

Важно: это не обязательно “отдельные модели” — это может быть отдельная роль в вашей multi-agent системе или просто формализованный шаг в CI/CD. Ценность — в **разделении ответственности**: исполнитель пишет/меняет, верификатор доказывает, тест‑раннер прогоняет и фиксирует результат.

> **Пример реализации (как это выглядит в инструменте):** многие среды позволяют настраивать специализированных исполнителей для “верификации”, “дебага” и “запуска тестов”. Например, см. раздел про “Verification agent” и “Test runner” в [Cursor Subagents](https://cursor.com/docs/context/subagents) (как один из примеров реализации концепта).

Дополнение: чтобы этот контур был устойчивым, удобно упаковать “верификацию” и “прогон тестов” как **навыки (skills)**: с чётким описанием “когда применять”, DoD/форматом отчёта и ссылками на команды/скрипты. Тогда проверка становится переносимой процедурой, а не разовой просьбой в чате.[^agentskills-overview]
...
- Точность (accuracy): 80% (16/20 correct)
- Доля ложных срабатываний (false positive rate): 15% (3/20 wrong)
...
- Точность (accuracy): 90% (18/20 correct) — **+10% улучшение**
- Доля ложных срабатываний (false positive rate): 10% (2/20 wrong) — **-5% улучшение**
- Средний TTRC: <TTRC_SECONDS> — **улучшение относительно baseline**
...
- В 2026 цикл улучшений: точность (accuracy) 80% → 90% (измеримо +10%)
...
Агент проходит eval (точность (accuracy) 90%) → деплой в продакшен → **падает на edge case** (например, network partition или редкий DB deadlock).

**Решение: матрица покрытия тестами (Test Coverage Matrix)**
...
1. **Network partition:**
   ...
   - Первопричина: misconfigured правила сети/egress (firewall)
...
- Рутинные сценарии (happy path): точность (accuracy) ≥ 95%
- Краевые случаи (edge cases): точность (accuracy) ≥ 70%
- Неизвестное (unknown): доля эскалаций = 100%

В 2014 Bill тестировал только рутинные сценарии (happy path) (routine incidents). В 2026 покрытие тестами включает edge cases → агент становится устойчивее.

### Концепция 5: continuous eval (непрерывный eval) — мониторинг качества в продакшене

**Проблема:**

Eval на стейджинге: точность (accuracy) 90%.  
Продакшен: точность (accuracy) 70% (хуже).
...
**Метрики eval в продакшене (real-time):**
...
- Если accuracy < 80% в течение 7 дней → расследовать регрессию
- Если false positive rate > 15% → пересмотреть плейбук триажа
- Если TTRC > 10 минут → оптимизировать диагностические команды

В 2014 Bill не мог измерять качество в продакшене (нет eval-контурa). В 2026 continuous eval → мониторинг качества в реальном времени.
...
Примечание: ниже — пример/псевдокод. Не запускайте команды в продакшене «как есть»: адаптируйте под вашу систему инцидентов и доступы (read‑only по умолчанию, allowlist команд).
...
**Исправление 1: добавить гипотезу network partition (разрыв сети)**

```diff
+### Гипотеза 5: network partition (разрыв сети)
+
+**Диагностика:**
+```bash
...
+  - ESCALATE к сетевой команде (network team)
```
...
- В 2026 настройка eval: 2 часа (собрать инциденты, написать eval script) → **переиспользуемо** для всех будущих прогонов
...
Точность (accuracy) агента 95% → деплой в продакшен → **падает на network partition** (не было в eval).

**Проблема:** eval-набор не репрезентативен (нет edge cases).

**Вывод:**

В 2014 Parts Unlimited тестировали Phoenix Project на стейджинге (happy path only). Продакшен выявил edge cases → серия outage (outages).

В 2026 eval-набор **должен включать edge cases**:

**Состав eval-набора (рекомендуемая пропорция):**
- **Рутинные сценарии (happy path):** 80% (например, high CPU, disk full)
- **Краевые случаи (edge cases):** 15% (например, network partition, deadlock, cascading failure)
- **Неизвестное (unknown):** 5% — агент должен эскалировать
...
**Проверка:** точность (accuracy) агента на edge cases ≥ 70% + доля эскалаций по unknown = 100%.

**Как избежать:**

Чеклист eval-набора:
- 80% — рутинные сценарии (happy path)
- 15% — краевые случаи (edge cases)
- 5% — неизвестное (unknown) (эскалация)
...
В 2026 golden tests **должны эволюционировать**:
...
Q1 2025: 3 инцидента “VIP не анонсируется / хост умер” (critical) → добавить в golden tests
Q2 2025: 5 проблем с инвалидацией Redis‑кэша (Redis cache invalidation issues) (часто) → добавить в golden tests
...
Разработчик меняет плейбук триажа → коммитит → **деплоит без eval** → в продакшене точность (accuracy) агента падает 90% → 75%.
...
В 2026 **eval обязателен в CI/CD**:
...
CI/CD gate checklist:
 ...
 - Деплой блокируется, если точность (accuracy) ниже порога
...
2. **Метрики:** точность (accuracy), доля ложных срабатываний (false positive rate), средний TTRC
3. **Golden tests:** 5 критичных инцидентов, которые должны проходить (защита от регрессий)
4. **Цикл улучшений:** measure → analyze failures → fix → re-measure
5. **Continuous eval:** инциденты из продакшена добавляются в eval-набор (он растёт со временем)

### Почему это работает

**В 2014:** Bill Palmer «чувствовал», что стало лучше. Нет метрик → нет доказательства и нет системного улучшения.

**В 2026:** eval-набор делает качество **измеримым**. Точность (accuracy) 80% → improvements → 90% → 95%. Каждое улучшение измеримо.
...
**Bill Palmer (2014):**
- Измерение качества: «чувствуем, что лучше» (subjective)
- Отслеживание улучшений (tracking): нет (нет метрик)
- Защита от регрессий (regression prevention): нет (без golden tests)

**С eval-набором (2026):**
- Измерение качества: точность (accuracy) 90%, TTRC <TTRC_SECONDS> (объективные метрики)
- Отслеживание улучшений: точность (accuracy) 80% → 90% (+10% измеримое улучшение)
- Защита от регрессий: golden tests блокируют деплой при регрессии
...
- Объяснить разницу между точностью (accuracy) и долей ложных срабатываний (false positive rate)
- Перечислить 3 типа инцидентов в eval-наборе: рутинные сценарии (happy path), краевые случаи (edge cases), неизвестное (unknown)
...
- Цикл улучшений работает (точность (accuracy) растёт со временем)

### Следующие шаги

**Глава 9:** Команда агентов + governance — как масштабировать от 1 агента к команде агентов (Analyst/Triage/SRE/Orchestrator).
...
- Eval script (посчитать метрики: точность (accuracy), доля ложных срабатываний (false positive rate), TTRC)
- Определение golden tests (критерии + контрольная точка в CI/CD)
- Процесс цикла улучшений (measure → analyze → fix → re-measure)
- Матрица покрытия тестами (Test coverage matrix): рутинные сценарии (happy path) 80%, краевые случаи (edge cases) 15%, неизвестное (unknown) 5%
