---
title: "Глава 1: Первый промпт + план верификации"
description: "Как написать первый промпт как контракт и проверить результат агента через план верификации, а не «поверить на слово»."
weight: 1
---

# Глава 1: Первый промпт + план верификации
...
**2026 год.** Познакомьтесь с Bishop.

Bishop — руководитель программы Phoenix Project (руководитель поставки) в компании, похожей на Parts Unlimited. Те же проблемы: Phoenix Project “не успевает”, деплои ломаются, инциденты повторяются, инфраструктура разрослась, а команда небольшая.

Но есть одно отличие: у Bishop есть ИИ-агенты — как «ускоритель поставки» и как способ сделать качество воспроизводимым.
...
## Та же задача в 2026: как Bishop решает с ИИ-агентами

**Контекст:** CEO Parts Unlimited (2026) задаёт тот же вопрос: "Почему деплои ломаются?"

**Подход Bishop (2026):**

Bishop открывает IDE-агент и формулирует промпт:
...
Bishop отвечает: "Да. План верификации прилагается. Проверено на выборке — совпадение. Хочешь проверить больше — вот список ID для выборочной проверки."
...
| **Проверяемо?** | Нет (поверь на слово) | Да (план верификации) |

**Что изменилось:**
- **Скорость:** заметно быстрее (дни ручной работы → быстрый прогон с проверкой)
- **Точность:** выше (план верификации обязателен)
- **Воспроизводимость:** промпт можно переиспользовать для следующих 50 деплоев
- **Проверяемость:** план верификации делает результат доверенным

**Что НЕ изменилось:**
- **Ответственность на человеке:** Bishop обеспечивает проверяемость результата (не доверяет слепо)
...
- верифицировать результаты агента (план верификации)
...
- Любой ИИ-агент: IDE-агент или чат-модель
...
1. **Скопируй промпт** в IDE‑агент (например, в Cursor) или чат‑модель (например, ChatGPT/DeepSeek/Claude)
...
6. **Проверь результат** (см. ниже "План верификации")
...
**Фикс:** добавили в промпт явный критерий: "латентность p95 выше порога считается проблемой".
...
В 2014 Bill Palmer потратил дни команды на ручной сбор данных о деплоях. Показал CEO: "падений много, вроде бы из-за миграций". CEO спросил: "Это точно?" Bill честно ответил: "Это лучшее, что мы можем собрать вручную. Точность… условная." Проблема: ошибки человека, смешанные форматы, качественные выводы.
...
План верификации делает результат проверяемым: выборочная проверка 2-3 случаев → 100% совпадение с оригинальными логами.
...
**Что делать:**
- Запусти агента
- Примени план верификации (выборочная проверка, граничные случаи, проверка здравого смысла)
...
### Как это стало в 2026 (Bishop, с агентами)
...
**Действия (что делает Bishop с агентом):**
- Bishop открывает IDE-агент, пишет промпт:
  ```
...
- План верификации: выборочная проверка → совпадение на выборке

**Затраты времени:** заметно меньше, чем ручной сбор

**Последствия:**
 - Результат проверяемый (автоматизация + план верификации)
...
| Аспект | Bill 2014 (начало) | Bill 2014 (конец, после автоматизации) | Bishop 2026 (с агентами) | Комментарий |
|--------|-------------------|-------------------------------------|--------------------------|-----------|
...
- Bishop добивается радикального ускорения через агентов
...
4. **Верификация встроена:** план верификации явно в промпте, не "забыли проверить"
...
- **В 2026:** Bishop — оркестратор, который пишет промпт и верифицирует результаты (системно и осмысленно)

**Не замена, а усиление:**

| Что делает | Bill 2014 (автоматизация) | Bishop 2026 (агенты) |
|------------|------------------------|----------------------|
...
- Умение ставить критерии качества: критерии готовности (Definition of Done, DoD) → план верификации
- Умение проверять результаты (ревью → доказательства через выборочную проверку)

Bill Palmer делал это с людьми. Bishop делает это с агентами.

**Принципы те же. Исполнение — в разы быстрее.**
...
**Как избежать:**
Всегда применяйте план верификации:
- Выборочная проверка: 2-3 кейса вручную
- Граничные случаи: первый/последний, min/max
- Проверка здравого смысла: статистика сходится

Bill Palmer в 2014 показал CEO данные с точностью ~70% — «лучшее, что мы можем собрать вручную». В 2026 с планом верификации вы отвечаете: «Проверено выборочной проверкой на 3 случаях — совпадение с оригинальными логами».
...
- После: заметно выше — автоматизированный парсинг + план верификации (выборочная проверка)
...
Шаблон для расчёта — в Приложении A (`book/appendix/A-business-case.md`).
...
Bishop получает инкрементальную ценность быстро (первый промпт).
...
- Команда видела: план верификации критичен («доверяй, но проверяй»)
...
- Риски: снижены через план верификации

**Преимущество агентов:**
- быстрее путь к первому циклу ценности (делегирование → проверка → повторяемость)
- **Воспроизводимо:** промпт работает всегда; аналитик может уволиться; инструмент может потребовать обновлений
...
3. Составьте план верификации (2-3 кейса, сравните с ручным результатом)
...
- План верификации работает → в Главе 2 добавим условия остановки (когда агент должен остановиться)
...
- **План верификации:** методы проверки результатов агента
