...
Он открывает инцидентный канал, добавляет ссылку на тикет, закрепляет два сообщения:

- “Что мы точно знаем?” (только факты)
- “Что мы считаем гипотезами?” (с пометкой уверенности)

Дальше Lance делает то, что десять лет назад делали руками, но без конференц‑колла на старте: он запускает несколько веток работы — каждая со своей задачей и строгим форматом результата. В Cursor это выглядит как делегирование задач отдельным помощникам в отдельных окнах контекста; Lance не думает об этом как о “технологии”, он думает об этом как о дисциплине: **чёткие входы → чёткий выход → проверка**.
...
```json
{
  ...
  "review": {"status": "STOP", "reason": "missing proof / requires human approval for changes"},
...
Первый вопрос у Ричарда не про “версии” и не про “кто виноват”. Он про доказательства: где подтверждение миграции. Ответ тоже в пакете: следующий шаг — безопасная проверка, а рискованные действия помечены STOP до подтверждения человека.
...
### Как выглядит skill (в нашем подходе)

Skill — это папка, где есть один “главный” документ и при необходимости справочные материалы:
...
## Артефакт результата
- decision packet (fact/hypothesis/risk/next_checks/STOP)
...
Через неделю появляется следующий PR/MR — и он уже не зависит от того, был ли Lance на дежурстве. Он зависит от того, что у команды есть skill, который можно менять по истории и ревью.

### Как skills версионируются (и почему это не «версии ПО»)
...
Версия — это commit. Если нужно — tag. Если меняем процедуру — делаем PR/MR и ревью. Если skill устарел — пишем это в `SKILL.md` и указываем замену.
...
А TRACE (см. главу 2) становится мостом между “ответом” и “процедурой”: когда ночью кто‑то принимает решение, утром можно увидеть, на каких skills и правилах оно стояло.
...
Этот раздел Lance добавляет в конце skill’а не как “страшилки”, а как память о том, как система ломается в реальности.
...
В чате есть ссылки, но нет картины: какие правила применялись, какие навыки подтягивались, что считалось доказательством. Ревью превращается в археологию.
...
### История 4: навыки остаются в чате

Когда skill живёт только в переписке, он умирает ровно в тот момент, когда меняется команда или инструмент. Через полгода новый инженер снова делает “как получится”, а в постмортеме снова появляются те же фразы:

— «Мы думали, что это и так очевидно».  

Вывод Lance: навыки должны жить в Git и меняться через ревью — так же, как код.
...
Перед тем как разойтись, Lance дописывает три строки в конец skill’а — как напоминание, которое потом спасёт не один инцидент:
...
- **Механика**: subagents дают изоляцию контекста, параллельность и делегирование (например, в Cursor).
...
4. Объяснить, почему skills должны жить в Git и как ссылаться на конкретную версию (commit/tag).
