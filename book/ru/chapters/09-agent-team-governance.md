...
**1) Изоляция контекста (отдельные контекстные окна)**  
...
Цена: у каждого исполнителя есть “стартовый overhead” — он начинает с чистого листа и не “помнит” вашу основную беседу. Поэтому оркестратор обязан делать корректную **передачу контекста (handoff)**.

**Минимальный контракт передачи контекста (что оркестратор передаёт исполнителю):**
...
Чтобы асинхронность не превратилась в хаос, у каждой фоновой ветки должен быть **идентификатор** (чтобы можно было “возобновить (resume)”) и понятный формат промежуточного статуса: что уже проверено и чего не хватает.

**3) Три режима управления командой**

- **Автоматическая маршрутизация (automatic delegation)**: оркестратор выбирает исполнителя по описанию роли и контексту (“это похоже на отладку тестов → зови отладчика/верификатора”).
- **Явный вызов (explicit invocation)**: человек/оркестратор явно назначает роль (“попроси ревьюера проверить риск данных”).
- **Параллельное исполнение**: независимые ветки запускаются одновременно (например, Analyst строит таймлайн, а Reviewer параллельно готовит риск‑матрицу).
