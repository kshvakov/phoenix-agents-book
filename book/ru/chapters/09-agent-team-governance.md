...
description: "Как сделать мультиагентную практику управляемой: делегирование, роли и проверки, Skill Router, Agent Skills, Git-first и анти‑паттерны."
lastmod: 2026-01-22
weight: 9
---
...
См. «место правды» для минимального контракта: [Приложение C — пакет решения: минимальный контракт]({{< relref "appendix/C-process-and-artifacts.md" >}}#decision-packet-contract).
...
Ниже — укороченный пример того, что Lance фиксирует после payroll‑инцидента (не как учебник, а как то, что завтра реально будут копировать и применять). В терминах Agent Skills это лежало бы в `payroll-triage/SKILL.md` (и `name` в frontmatter должен совпадать с именем директории):
