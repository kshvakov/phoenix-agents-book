...
В некоторых реализациях навыков встречается идея “`allowed-tools`” — перечисление заранее разрешённых инструментов для навыка. Даже если конкретная платформа это не поддерживает буквально, как концепт это полезная часть baseline: ограничить поверхность атаки и обеспечить воспроизводимость.[^agentskills-allowed-tools]
