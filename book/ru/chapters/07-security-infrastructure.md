...
- **Разрешённый список (allowlist)**: явно ограничить, какие команды/инструменты допустимы для навыка (по умолчанию запрещено всё).
- **Подтверждение (approval)**: опасные операции — только с явным подтверждением человека/процесса.
- **Аудит**: логировать запуск скриптов так же, как `ssh`/`sudo`/`ansible`, чтобы это было пригодно для расследований.

В некоторых реализациях навыков встречается концепт «разрешённые инструменты» (`allowed-tools`) — перечисление заранее разрешённых инструментов для навыка. Поддержка формата зависит от реализации, но как идея это полезная часть baseline: ограничить поверхность атаки и обеспечить воспроизводимость.[^agentskills-allowed-tools]
...
[^agentskills-allowed-tools]: См. поле `allowed-tools` и рекомендации по интеграции/безопасности: [Agent Skills Specification](https://agentskills.io/specification), [Integrate skills](https://agentskills.io/integrate-skills).
...
**Митигация:**
- Sudoers allowlist: агенту разрешены только конкретные команды (например, `systemctl restart <ALLOWLISTED_SERVICES>`) и только на конкретных хостах
...
Примечание: плейсхолдеры вида `<WINDOW>/<THRESHOLD>/...` — это значения‑заглушки в шаблонах. См. глоссарий — [«Нотация плейсхолдеров»]({{< relref "appendix/glossary.md" >}}#placeholder-notation).
...
**До Кейса 2 (после Кейса 1):**
- **Скорость:** порядка 120 story points/спринт (оценка; +20% от baseline)
...
- **Скорость:** порядка 180 story points/спринт (оценка; **+80%** к baseline, +50% к кейсу 1) — первые признаки «экспоненты» уже видны
...
- Скорость команды: ~180 → ~240 story points/спринт (оценка; +140% суммарно от baseline)
...
- С моделью угроз + планом изменений (2026): анализ угроз сделан, тест на стейджинге <WINDOW>, время отката < <WINDOW>, простой 0, цена — потери предотвращены
