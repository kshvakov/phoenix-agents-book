...
## Та же задача в 2026: как Bishop решает с агентом

**Контекст:** Bishop в Parts Unlimited 2026 столкнулся с той же проблемой — есть senior-инженер (Richard Hendricks), который знает всё о легаси‑БД. Но Richard уходит в отпуск на 3 недели.

**Задача до отпуска:** зафиксировать процесс разработки изменения так, чтобы команда могла делать это без Richard.

**Решение Bishop:** создать SOP "дизайн → PR" с агентом, который может воспроизвести процесс Richard.
...
5) План отката: как откатить если что-то сломается?
...
- Граничные случаи: ✅
- Обработка ошибок: ✅
- Откат: ⚠️ не протестировано → **СТОП**, сначала протестируй откат
...
- Если откат не протестирован → СТОП, сначала протестируй откат
...
Bishop: "Richard, тесты прошли локально — можно в продакшен?"
...
Patty McKee после этого ввела правило: "Каждое изменение должно иметь план отката **до деплоя**."

В 2026 контрольная точка 4 требует план отката:

```
контрольная точка 4: PR

**План отката:**
- Команда: `ansible-playbook -i inventories/production playbooks/report-service.yml --tags rollback --extra-vars "report_service_version=<PREV_VERSION>"`
- Оценка времени: < 5 min
- Проверка результата: смоук‑тест после отката
- **Tested:**  rollback протестирован на staging

**УСЛОВИЕ ОСТАНОВКИ:** если откат не протестирован → СТОП, сначала протестируй откат на стейджинге
```

**Как избежать:**

SOP делает rollback **частью процесса**:
- Агент генерирует команды отката
- Агент тестирует rollback на staging (forward → rollback → verify)
- Только если rollback работает → одобрить деплой в продакшен
...
### Как это стало в 2026 (Bishop, knowledge capture с агентами)
...
**Подход Bishop (2026): "Парное программирование с агентом для фиксации знаний"**
...
- **В 2026:** Bishop использует документацию + наставничество **+ агентов** (быстро, автоматизировано, самообновляется)

**Не замена, а усиление:**

| Что делает | Bill 2014 (подход через документацию) | Bishop 2026 (агенты для фиксации знаний) |
|------------|----------------------------------|------------------------------------------|
...
**Bishop (2026) делегирует агенту через SOP:**
...
- Bishop учит агента через создание SOP (делегирование процесса)
...
4. **Контрольная точка 4 (PR):** ревью + план отката + критерии готовности
...
- Изменение ревьюируемо, тестируемо, и откат протестирован
