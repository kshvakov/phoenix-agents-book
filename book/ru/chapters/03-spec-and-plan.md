...
Примечание: в примерах ниже используются плейсхолдеры вида `<WINDOW>/<THRESHOLD>/...`. См. глоссарий — [«Нотация плейсхолдеров»]({{< relref "appendix/glossary.md" >}}#placeholder-notation).
...
В индустрии это оформляют как **навыки (skills)**: папка с `SKILL.md` (инструкции и метаданные) и, при необходимости, `references/` (справки), `assets/` (шаблоны) и `scripts/` (повторяемые проверки). Ключевой принцип — “подгрузка по требованию” (progressive disclosure): агент видит список навыков по короткому описанию, а подробности грузит только когда выбран нужный навык (экономия контекста и меньше шума).[^agentskills-spec]
