...
- качество контролируется гейтами (план проверки, stop conditions, ревью)
...
- baseline/quality gates для команд
...
- **Таск‑трекер** — управление работой: статус/обсуждение + ссылки на конкретные версии артефактов в Git + evidence.
...
**Гипотеза:** если эксперт делает типовую задачу “вслух”, агент может превратить её в `SOP` с контрольными точками и stop conditions; дальше команда выполняет задачу по `SOP`, а эксперт становится ревьюером артефакта, а не “единственным исполнителем”.
...
- запрет на действия вне `allowlist`
...
- закрепить владельца артефактов (baseline, quality gates, шаблоны)
...
- доля изменений, которые отклонены гейтом (и почему)
...
- bus factor (baseline → target) как наблюдаемая зависимость от конкретных людей
