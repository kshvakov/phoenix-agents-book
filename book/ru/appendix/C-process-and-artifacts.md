...
```mermaid
flowchart TD
  Idea[Идея_и_ставки] --> Spec[Спецификация]
  Spec --> Plan[План]
  Plan --> Arch[Архитектура]
  Arch --> Impl[Реализация_через_SOP]
  Impl --> Verify[Проверка_и_верификация]
  Verify --> Deploy[Деплой_и_откат]
  Deploy --> Ops[Операции_и_инциденты]
  Ops --> Improve[Улучшения]
  Improve --> Spec
```
