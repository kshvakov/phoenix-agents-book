...
Практическая ремарка: план — это не “черновик в чате”, а **отдельный ревьюируемый артефакт**. Его удобно править и переигрывать цикл исполнения: если агент делает “не то”, часто быстрее вернуться к плану, уточнить входы/ограничения/DoD и повторить, чем латать результат перепиской. См. [Cursor: Best practices for coding with agents](https://cursor.com/blog/agent-best-practices) (разделы про планы и «starting over from a plan»).
...
Ремарка: на практике “fresh start” чаще всего реализуется как **возврат к плану** (plan‑first): уточнить требования/ограничения, обновить план и перезапустить выполнение по нему, вместо того чтобы чинить результат цепочкой уточнений. См. [Cursor: Best practices for coding with agents](https://cursor.com/blog/agent-best-practices).
