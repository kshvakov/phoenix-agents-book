...
- **TRACE**: блок “аудита контекста” в ответе агента: что именно было реально прочитано и использовано (правила, навыки, ссылки на артефакты) — чтобы можно было понять, на чём стояли выводы и как воспроизвести результат.
...
- **Agent Skills**: открытый формат “навыков” — переносимых пакетов знаний и процедур для агентов. Навык живёт в папке, содержит `SKILL.md` (метаданные + инструкции) и может включать `scripts/`, `references/`, `assets/`.

- **`SKILL.md`**: основной файл навыка (skill) с YAML frontmatter (`name`, `description` и др.) и инструкциями в Markdown.

- **Progressive disclosure (подгрузка по требованию)**: принцип экономии контекста: сначала агент видит только метаданные навыков (когда использовать), затем подгружает полный текст `SKILL.md` и дополнительные ресурсы только для выбранного навыка.
