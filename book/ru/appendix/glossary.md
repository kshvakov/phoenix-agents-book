...
- **Формат:** `<UPPER_SNAKE_CASE>` без пробелов внутри (например, `<WINDOW>`, а не `< WINDOW >`).
...
- **Progressive disclosure (подгрузка по требованию)**: принцип экономии контекста: сначала агент видит только метаданные Agent Skills (когда использовать), затем подгружает полный текст `SKILL.md` и дополнительные ресурсы только для выбранного Agent Skill.
...
- **`read_only`**: режим работы, в котором исполнитель имеет право только читать/собирать данные и формировать артефакты, но **не имеет права** менять состояние систем (особенно production). Любые действия, меняющие состояние, требуют явного разрешения (approval) и/или отдельного контура исполнения.
