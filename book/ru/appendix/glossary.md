...
- **Пакет решения (decision packet)**: краткий проверяемый артефакт, который отделяет факты от гипотез, фиксирует риск и явно показывает, что требует подтверждения (approval) и какие проверки/следующие шаги планируются. В тексте книги: при первом упоминании пишем «пакет решения (decision packet)», дальше — «пакет решения».
