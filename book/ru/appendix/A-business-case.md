...
**Как будем снижать риски:** ограничения (guardrails), условия остановки, планы проверки, минимальные привилегии (least privilege), журнал аудита, eval-набор и эталонные тесты (golden tests).
...
- **Митигации:** ревью человеком, разрешённый список действий, консервативная эскалация при неопределённости (uncertainty), сухой прогон (dry‑run) плейбука (`--check`) + канареечный/постепенный rollout (`serial`) + план отката (rollback), аварийный выключатель (kill switch)
...
- **Митигации:** маскирование данных, сканирование секретов (secret scanning), запрет на публикацию сырых логов, безопасные каналы
...
- governance (управление практикой) и базовый уровень безопасности — часть проекта, а не “потом”
