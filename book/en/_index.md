---
title: "Autonomous Agents: transforming engineering and business"
description: "A book about transforming engineering practice with autonomous AI agents: from prompts and SOPs to quality, security, and change management."
lastmod: 2026-01-21
---

# Autonomous Agents: transforming engineering and business

In the style of *The Phoenix Project*: from chaos to effectiveness with AI agents.

> **Status:** Work in Progress. The structure is stable; chapters are being edited and refined.

---

## How to read this book

**Fast start for engineers:**

1. Start with [Chapter 1]({{< relref "chapters/01-first-prompt.md" >}}) — create your first working prompt and verification plan
2. Use the “Quick start” section in each chapter for immediate practical value
3. Come back to the full chapter when you need depth

**For managers and business stakeholders:**

- Start with the short summary at the beginning of each chapter
- Read [Appendix A: Business case template]({{< relref "appendix/A-business-case.md" >}}) as a template for justification
- Read [Appendix B: Organizational transformation]({{< relref "appendix/B-organizational-transformation.md" >}}) as a change-management playbook

---

## Structure: 3 cases, 10 chapters

### Case 1: deployment chaos (Chapters 1–4)

- [Chapter 1: first prompt]({{< relref "chapters/01-first-prompt.md" >}})
- [Chapter 2: system prompt and guardrails]({{< relref "chapters/02-system-prompt-guardrails.md" >}})
- [Chapter 3: from a business request to spec v1 + plan v1]({{< relref "chapters/03-spec-and-plan.md" >}})
- [Chapter 4: architecture design v1]({{< relref "chapters/04-architecture-v1.md" >}})

### Case 2: the Brent bottleneck (Chapters 5–7)

- [Chapter 5: SOP “design -> PR”]({{< relref "chapters/05-sop-design-to-pr.md" >}})
- [Chapter 6: operations and incidents]({{< relref "chapters/06-operations-incidents.md" >}})
- [Chapter 7: security and infrastructure]({{< relref "chapters/07-security-infrastructure.md" >}})

### Case 3: the payroll incident (Chapters 8–10)

- [Chapter 8: eval dataset + golden tests]({{< relref "chapters/08-eval-golden-tests.md" >}})
- [Chapter 9: agent teams + governance]({{< relref "chapters/09-agent-team-governance.md" >}})
- [Chapter 10: capstone (full cycle)]({{< relref "chapters/10-capstone-full-cycle.md" >}})

